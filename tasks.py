...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_POC_CORE_MODEL_EXTENSION_xxx
namespace = Collection("poc_core_model_extension")
namespace.configure(
    {
        "poc_core_model_extension": {
            "nautobot_ver": "2.3.1",
            "project_name": "poc-core-model-extension",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.poc_core_model_extension.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.poc_core_model_extension.compose_http_timeout,
        "NAUTOBOT_VER": context.poc_core_model_extension.nautobot_ver,
        "PYTHON_VER": context.poc_core_model_extension.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.poc_core_model_extension.project_name}",
        f'--project-directory "{context.poc_core_model_extension.compose_dir}"',
    ]

    for compose_file in context.poc_core_model_extension.compose_files:
        compose_file_path = os.path.join(context.poc_core_model_extension.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.poc_core_model_extension.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.poc_core_model_extension.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.poc_core_model_extension.python_ver
    dockerfile_path = os.path.join(context.poc_core_model_extension.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.poc_core_model_extension.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.poc_core_model_extension.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.poc_core_model_extension.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations poc_core_model_extension"
...
@task(
    help={
        "version": "Version of POC Core Model Extension to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} poc_core_model_extension"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("poc_core_model_extension") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " poc_core_model_extension.migrations"
        )
...
def unittest(  # noqa: PLR0913
    context,
    keepdb=False,
    label="poc_core_model_extension",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'poc_core_model_extension/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.poc_core_model_extension.local):
